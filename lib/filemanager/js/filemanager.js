...
var fmLib =  {
  ...
  inputFocus: false,
	...
	viewMenu: function(id, curCont) {
		...
		switch(id) {
			...
			case 'bulkAction':
				...
				if(userPerms.getFileMeta) {
					if(ids.length > 0) items.push({icon:'getfilemeta.png',caption:fmMsg[lang].cmdGetFileMeta,exec:['fmLib.getMetaCheckedFiles',curCont,ids]});
					else items.push({icon:'download_x.png',caption:fmMsg[lang].cmdGetFileMeta});
				}
				else if(!userPerms.hideDisabledIcons) {
					items.push({icon:'download_x.png',caption:fmMsg[lang].cmdGetFileMeta});
				}
	...
	getMetaCheckedFiles: function(curCont, ids) {
		var ajaxObj = new ajax();
		if(!ids) ids = fmTools.getSelectedItems(curCont);
		else if(typeof ids != 'object') ids = [ids];

		if(ids.length > 0) {
			var url = fmWebPath + '/action.php?fmContainer=' + curCont + '&fmMode=getMeta&fmObject=' + ids.join(',');
			ajaxObj.makeRequest(url, function() {
				if(!ajaxObj.response) {
					alert('ERROR: Could not load explorer!');
					return false;
				}
				//console.log(JSON.parse(ajaxObj.response), document.getElementById('targetEl'));
				window.parent.triggerImport(ajaxObj.response, 'documents');
