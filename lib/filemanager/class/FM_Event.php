...
/* PUBLIC METHODS ****************************************************************************** */
	...
	public function handle($type, $id = '', $param = '') {
		switch($type) {
			...
			case 'getMeta':
				$files = $this->_getFileMetaData($id);
...
/* PROTECTED METHODS *************************************************************************** */
	...
	protected function _getUserPerms() {
		...
		print 'getFileMeta:' . (int) $this->_FileManager->enableGetFileMeta . ',';
	...
	/**
	 * get meta data of selected files
	 *
	 * @param array $ids		entry IDs
	 * @return array
	 */
	protected function _getFileMetaData($ids) {
		/////////// this function is not used / does not work as expected ////////////
		$files = $dirs = $fileinfo = array();
		$ids = explode(',', $ids);
		
		//$this->_Listing->refresh($ids);
		//$this->_FileManager->Log->add("Meta file $ids");

		if($this->_FileManager->enableGetFileMeta) {
			if(is_array($ids)) foreach($ids as $id) {
				if($Entry = $this->_Listing->getEntry($id)) {
					if(!$Entry->isDir()) {
						$fileinfo[] = mime_content_type($Entry->path);
					}
				}
			}
		}
