  ...
  var reformed = {
    ...
    applyArrayEditingEvents: function(el) {
      ...
      $(el).find('.array > .actions > a.another').click(function() {
        ...
        var appended = actions.prev('fieldset, input').clone().appendTo(array);
        
        appended.find('.front > fieldset').coolfieldset();
        field = appended.closest('.kvp').attr('id')
        appended.addClass('clone');
        nextClone = Number(actions.find('.another').attr('data-count')) + 1;
        actions.find('.another').attr('data-count', nextClone);
        nextPicker = field+'-'+nextClone;
        prevPicker = field+'-'+(nextClone-1);
        
        //begin picker functionality
        //get parent elements and values for clone
        $fieldset = appended.closest('fieldset.object')
		$kvp = $fieldset.closest('.kvp')
        $namefield = $fieldset.find('.kvp#name > .front')
        datapath = $kvp.attr('data-path')
        
        //set cloned picker elements
        appended.find('.saver').attr('data-index',nextClone);
        appended.find('.savestatus').html('')
        appended.find('.kvp#name').on('keyup', 'input.value', function() {
            $(this).closest('fieldset.object').find('.pickername').val($(this).val())
        });
        
        //activate picker functionalities for cloned
        if ($('.ssi-'+field).length || $('.ssi-'+prevPicker).length) {   // clone picker only if an initial instance exists
            if (appended.attr('data-field') == 'document') {
                datafield = 'document'
            } else {
                datafield = 'organisation'
            }
            appended.find('.dialog-picker').remove();  // remove cloned instance (for documents)
            clonedpicker = $('<div class="ssi-'+nextPicker+' dialog-picker '+datafield+'" data-index="'+nextClone+'" data-field="'+datafield+'"></div>')
            appended.find('.pickerwrap').prepend(clonedpicker)
            setCSS(appended.find('.dialog-picker'), appended.find('.saver'));
            fieldManager(nextPicker, datapath)
        }

        if (appended.is('fieldset')) {
          appended.find('input:not(.altkey)').val('');
          appended.find('textarea').val('');
        } else {
          appended.val('');
        }
    ...
    kvp: function(key, value) {
      ...
      var output = '<div class="kvp" id="'+key+'"><div class="front">';
      if (reformed.settings.editor == 'edit') {
        output += '<span class="handle">drag</span> ';
        output += '<input type="text" value="'+key+'" class="key" />';
      } else {
        noncamelkey = key.replace(/([A-Z])/g, ' $1').replace(/^./, function(str){ return str.toUpperCase(); })
        output += '<label class="key '+key+'">'+noncamelkey+'<input type="hidden" value="'+key+'" class="altkey"></label>';
      }
      ...
      if (use_schema) {
        ...
        if ($.isPlainObject(value)) {
          ...
          output += reformed.array(value, key);
          ...
          &&
            ...
            output += '<input class="value '+type+'" type="text" value="'+value+'" />';
          }
    ...
    array: function(array, items) {
      ...
      if (reformed.settings.editor == 'edit' || reformed.settings.editor == 'data') {
        output += '<div class="actions"><a class="another" data-count=1>+</a></div>';
      }
      output += '</fieldset>';
      return output;
    }
...
function kvp(el) {
  ...
  if (reformed.settings.editor == 'edit') {
    ...
    var v = $(el.find('div.front > input, div.front > textarea, div.front > select, fieldset')[1]);
  } else {
    var k = el.find('div.front > label.key > input.altkey').val();
    var v = $(el.find('div.front > input, div.front > textarea, div.front > select, fieldset')[0]);
  }
  if (v.is('input') || v.is('textarea') || v.is('select')) {
    if (v.is(':checkbox')) {
      _kvp[k] = v.is(':checked');
    } else {
      var value = v.val();
      //remove commas added from mask
      //console.log(value)
      value = value.replace(/\,/g,'');
      if (value) {
        _kvp[k] = (isNaN(value) ? value : parseFloat(value));
      } else {
        _kvp[k] = value;
      }
    }
